...
export const DEFAULT_ROLE_CATS: RoleCategory[] = ALL_ROLE_CATS.filter(c => c !== 'self')

// ── Persisted settings ────────────────────────────────────────────────────────
const LS_DEFAULT_SELF = 'rg_defaultSelfEnabled'
const defaultSelfEnabled = ref<boolean>(
  localStorage.getItem(LS_DEFAULT_SELF) === 'true'
)
watch(defaultSelfEnabled, v => localStorage.setItem(LS_DEFAULT_SELF, String(v)))

function toggleDefaultSelf(): void { defaultSelfEnabled.value = !defaultSelfEnabled.value }

/** Role cats to use when initialising a new slot, honouring the default-self preference. */
...
const personRoleFilters = ref<RoleCategory[][]>(_hot.personRoleFilters                  ?? [[...DEFAULT_ROLE_CATS], [...DEFAULT_ROLE_CATS]])
...
const activePeople = computed(() =>
  slots.value.filter((s): s is TmdbPerson => isPersonSlot(s))
)

/** Non-null title slots (title mode). */
const activeTitles = computed(() =>
  slots.value.filter((s): s is TmdbTitle => s !== null && !isPersonSlot(s))
)

const selfEnabled = computed(() =>
  personRoleFilters.value.every(f => f.includes('self'))
)

const personRoleCounts = computed(() =>
  credits.value.map(arr => {
    const c: Record<RoleCategory, number> = { ...EMPTY_ROLE_COUNTS }
    for (const p of arr) for (const cat of p.roleCategories) c[cat]++
    return c
  })
)
...
const filteredCredits = computed<Project[][]>(() =>
  credits.value.map((arr, i) => {
    ...
    if (allowed.length === ALL_ROLE_CATS.length) return arr
    return arr.filter(p => p.roleCategories.some(c => allowed.includes(c)))
  })
)

/** Exclusive Venn regions for person mode (projects per region). */
const regions = computed(() =>
  hasResults.value && searchMode.value !== 'title'
    ? computeRegions(filteredCredits.value)
    : new Map()
)
...
const filteredCastLists = computed<CastMember[][]>(() =>
  selfEnabled.value
    ...
    : castLists.value.map(arr =>
        arr.filter(m => m.roleCategories.some(c => c !== 'self'))
      )
)
...
const titleRegions = computed(() =>
  hasResults.value && searchMode.value === 'title'
    ? computeRegionsCast(filteredCastLists.value)
    : new Map()
)

// Reset selected region whenever the enabled set changes
watch(enabledMask, () => { selectedMask.value = allMask.value })
...
function addSlot(): void {
  if (slots.value.length >= MAX_PERSONS) return
  pushSlot()
  hasResults.value = false
}

function removeSlot(): void {
  if (slots.value.length <= MIN_PERSONS) return
  popSlot()
  hasResults.value = false
}
...
function removeSlotAt(idx: number): void {
  if (slots.value.length > MIN_PERSONS) {
    ...
    slots.value     = slots.value.map((s, i) => i === idx ? null : s)
    credits.value   = credits.value.map((c, i) => i === idx ? [] : c)
    castLists.value = castLists.value.map((c, i) => i === idx ? [] : c)
  }
  hasResults.value = false
  if (slots.value.every(s => s === null)) searchMode.value = null
}
...
function handleSlotUpdate(idx: number, val: TmdbPerson | TmdbTitle | null): void {
  ...
  } else if (s.every(slot => slot === null)) {
...
function updateRoleFilter(idx: number, val: RoleCategory[]): void {
  personRoleFilters.value = personRoleFilters.value.map((f, i) => i === idx ? val : f)
}

function toggleSelf(): void {
  if (selfEnabled.value) {
    personRoleFilters.value  = personRoleFilters.value.map(f => f.filter(c => c !== 'self'))
    defaultSelfEnabled.value = false
  } else {
    personRoleFilters.value  = personRoleFilters.value.map(f =>
      f.includes('self') ? f : [...f, 'self']
    )
...
function togglePerson(i: number): void {
  if (isToggleDisabled(enabledMask.value, i)) return
...
export function useVennState() {
  return {
    // ── State ─────────────────────────────────────────────────────────────────
    slots, credits, castLists, personRoleFilters,
    hasResults, selectedMask, enabledMask, searchMode,
    // ── Derived ───────────────────────────────────────────────────────────────
    allMask, activePeople, activeTitles,
    selfEnabled, personRoleCounts, filteredCredits, filteredCastLists, regions, titleRegions,
    // ── Actions ───────────────────────────────────────────────────────────────
    addSlot, removeSlot, removeSlotAt, compactSlots, handleSlotUpdate, clearSearch,
    updateRoleFilter, toggleSelf, togglePerson,
    defaultSelfEnabled, toggleDefaultSelf,
    setSlots, applyPersonResults, applyTitleResults,
  }
}
