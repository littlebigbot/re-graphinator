 ...
 */
export function useClickOutside(refs: Ref<HTMLElement | null>[], onOutside: () => void): void {
  const handler = (e: MouseEvent) => {
    const clickedInside = refs.some((elRef) => elRef.value?.contains(e.target as Node));
    if (!clickedInside) {
      onOutside();
    }
  };
