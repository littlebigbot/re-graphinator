...
interface HotData {
  creditsCache?: Map<string, CacheEntry>;
}
...
export function useCredits(apiKey: Ref<string>) {
  ...
  async function fetchAll(people: TmdbPerson[]): Promise<Project[][]> {
    return withLoading(async () => {
      const results = await Promise.all(people.map((person) => fetchCredits(person.id)));
  ...
  async function fetchAllCast(titles: TmdbTitle[]): Promise<CastMember[][]> {
    return withLoading(() => Promise.all(titles.map((title) => fetchCast(title.id, title.media_type))));
  }
