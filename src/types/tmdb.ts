...
export const EMPTY_ROLE_COUNTS: Record<RoleCategory, number> = {
  actor: 0, self: 0, director: 0, writer: 0, crew: 0,
}
...
function computeRegionsBase<TItem extends { id: number; roles: string[] }, TEnriched>(
  ...
  for (let i = 0; i < arrays.length; i++) {
    for (const c of arrays[i]) {
      if (!c.id) continue
      const key = uid(c)
      maskMap.set(key, (maskMap.get(key) ?? 0) | (1 << i))
      if (!dataMap.has(key)) dataMap.set(key, enrich(c))
      assign(dataMap.get(key)!, i, c.roles)
    }
  }

  for (const [key, mask] of maskMap) {
    if (!result.has(mask)) result.set(mask, [])
...
export function computeRegions(
  creditArrays: Project[][],
): Map<RegionMask, ProjectWithRoles[]> {
  return computeRegionsBase(
    creditArrays,
    c => `${(c as Project).media_type}-${c.id}`,
    c => ({ ...(c as Project), rolesByPerson: {} }),
    (r, i, roles) => { r.rolesByPerson[i] = roles },
  )
}
 ...
 */
export function computeRegionsCast(
  castArrays: CastMember[][],
): Map<RegionMask, CastMemberInRegion[]> {
  return computeRegionsBase(
    castArrays,
    c => `person-${c.id}`,
    c => ({ ...(c as CastMember), rolesBySlot: {} }),
    (r, i, roles) => { r.rolesBySlot[i] = roles },
  )
}
