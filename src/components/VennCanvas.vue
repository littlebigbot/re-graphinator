...
const tooltip = ref<{
  visible: boolean; x: number; y: number; names: string[]; count: number
...
function draw(): void {
  ...
  if (!el || !props.slots.length) return
  ...
  centers.forEach(({ x, y, origIdx }) => {
    ...
    if (!slot) return
    const imgUrl = isPersonSlot(slot)
      ? profileUrl(slot.profile_path)
      : posterUrl((slot as TmdbTitle).poster_path)
    ...
    if (imgUrl) {
      ...
      const g  = svg.append('g')
        ...
        .attr('pointer-events', 'none')
      g.append('circle').attr('cx', 30).attr('cy', 28).attr('r', 16)
      g.append('path').attr('d', 'M2 88 C2 58 58 58 58 88')
    }
  })
  ...
  function maskAt(mx: number, my: number): RegionMask {
    ...
    for (const { x, y, origIdx } of centers) {
      if (Math.hypot(mx - x, my - y) <= r) mask |= (1 << origIdx)
    }
    return mask
  }
  ...
  if (props.hasResults && props.selectedMask > 0) {
    const selCenters = centers.filter(c => (props.selectedMask >> c.origIdx) & 1)
    if (selCenters.length >= 2) {
      selCenters.forEach(({ x, y }, idx) => {
        ...
        if (idx > 0) circ.attr('clip-path', `url(#vsel-${idx - 1})`)
      })
      svg.append('rect')
  ...
  function updateHoverSliver(hoveredMask: RegionMask): void {
    ...
    if (popcount(hoveredMask) < 2 || hoveredMask === props.selectedMask) return

    const hovCenters = centers.filter(c => (hoveredMask >> c.origIdx) & 1)
    hovCenters.forEach(({ x, y }, idx) => {
      const cp   = defs.append('clipPath').attr('id', `vhov-${idx}`)
      const circ = cp.append('circle').attr('cx', x).attr('cy', y).attr('r', r)
      if (idx > 0) circ.attr('clip-path', `url(#vhov-${idx - 1})`)
    })
    hoverSliverGroup.append('rect')
      .attr('x', 0).attr('y', 0).attr('width', W).attr('height', H)
      .attr('clip-path', `url(#vhov-${hovCenters.length - 1})`)
      .attr('fill', 'white').attr('fill-opacity', 0.15)
      .attr('pointer-events', 'none')
  }
    ...
    circles.forEach((circle, j) => {
      ...
      if (selected && enabled) {
        ...
        circle
          ...
          .attr('fill-opacity',   inHover ? 0.10  : 0)
  ...
  svg.on('mousemove', function (this: SVGSVGElement, event: MouseEvent) {
    ...
    const count = mask > 0
      ? [...props.regionCounts.entries()]
          .filter(([r]) => (r & mask) === mask)
          .reduce((sum, [, n]) => sum + n, 0)
      : 0
    if (mask > 0 && count > 0) {
      const names = centers
        .filter(c => (mask >> c.origIdx) & 1)
        .map(c => surname(props.slots[c.origIdx]!.name))
  ...
  svg.on('click', function (this: SVGSVGElement, event: MouseEvent) {
    ...
    const [mx, my] = d3.pointer(event, this)
    const mask = maskAt(mx, my)
    if (mask > 0) emit('select', mask === props.selectedMask ? 0 : mask)
  })

  // ── Text helper ──
  type TextSel = d3.Selection<SVGTextElement, unknown, null, undefined>
  const T = (x: number, y: number, fill: string, size: number, weight: number, opacity = 1): TextSel =>
    svg.append('text')
      .attr('x', x).attr('y', y)
  ...
  centers.forEach(({ origIdx, angle }) => {
    ...
    T(lx, ly, PERSON_COLORS[origIdx], 13, 600)
      .text(label)
  ...
  if (props.hasResults && props.selectedMask > 0) {
    const selCenters = centers.filter(c => (props.selectedMask >> c.origIdx) & 1)
    const cx = selCenters.reduce((s, c) => s + c.x, 0) / selCenters.length
    const cy = selCenters.reduce((s, c) => s + c.y, 0) / selCenters.length
    const selCount = [...props.regionCounts.entries()]
      .filter(([m]) => (m & props.selectedMask) === props.selectedMask)
      .reduce((s, [, n]) => s + n, 0)
    const selColor = selCenters.length === 1 ? PERSON_COLORS[selCenters[0].origIdx] : '#c8b4e8'
    T(cx, cy, selColor, 30, 800, 0.95).text(selCount)
  }
}

onMounted(draw)
watch(
  () => [props.slots, props.hasResults, props.regionCounts, props.enabledMask, props.selectedMask] as const,
  draw,
)
</script>
...
.tip-count { font-size: 1.05rem; font-weight: 800; color: #fff; line-height: 1; }
.tip-label { font-size: 0.72rem; color: rgba(255, 255, 255, 0.55); line-height: 1; }

.tip-enter-active, .tip-leave-active { transition: opacity 0.1s ease, transform 0.1s ease; }
.tip-enter-from, .tip-leave-to { opacity: 0; transform: translateY(3px); }
</style>
