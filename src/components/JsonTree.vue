...
const isExpandable = computed(() =>
  valueType.value === 'array' || valueType.value === 'object'
)

const entries = computed((): [string, unknown][] => {
  if (!isExpandable.value || props.value === null) return []
...
const summary = computed(() => {
  switch (valueType.value) {
    ...
    case 'string': return `"${props.value}"`
...
const leafDisplay = computed(() => {
  if (valueType.value === 'string') return `"${props.value}"`
...
.jt-toggle:hover { background: rgba(255, 255, 255, 0.06); }

.jt-children { padding-left: 14px; border-left: 1px solid rgba(255,255,255,0.07); margin-left: 5px; }

.jt-arrow   { color: #666; font-size: 0.65rem; width: 10px; display: inline-block; }
.jt-key     { color: #9cdcfe; }
.jt-summary { color: #555; }

/* primitive colours — VS Code Dark+ inspired */
.jt-string  { color: #ce9178; }
.jt-number  { color: #b5cea8; }
.jt-boolean { color: #569cd6; }
.jt-null    { color: #569cd6; font-style: italic; }
</style>
